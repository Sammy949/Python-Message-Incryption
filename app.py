...
while True:
    ...
    if task in ("encrypt", "Encrypt"):
        ...
        messagebox.showinfo(
            "Encrypted Message",
            f"Message in encrypted version is: {encrypted}\n\nThe message has been copied to the clipboard!",
        )

    elif task in ("decrypt", "Decrypt"):
        message = get_message()
        decrypted = swap_letters(message)
        decrypted = remove_extra_character(
            decrypted
        )  # Remove the extra character if present
        messagebox.showinfo(
            "Decrypted Message", f"Message in decrypted version: {decrypted}"
        )
